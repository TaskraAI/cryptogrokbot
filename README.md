...
Pixel-art ops board for the six GrokBot crew agents (Chief, Scout, Sentinel, Grok, Scholar, Auditor). Shows live pulses, open trades, buys/sells, P&L, and major decisions.

```bash
npm install
npm run agent                 # desk + APIs on :8787
npm run farm                  # work farm UI on :5173 (proxies /api → :8787)
```

Open `http://localhost:5173`. Without a running agent, the farm runs a faithful offline simulation of the same six agents.
