...
function rungJobs(rung: ReturnType<typeof currentRung>): string[] {
  ...
  if (rung.multiple >= 10) {
    return [
      ...
      "Crypto only: Scout queues hype+volume. The desk never trades. Do not live-buy until Taskra, Chief, or team Approves.",
  ...
  return [
    ...
    "Crypto: Grok Bot Bearer only. The desk never auto-trades. Live buy needs an Approve. Size stays at policy maxSolPerTrade until Taskra raises it.",
...
export function playbook(opts: {
  ...
  return {
    ...
    tonight: [
      ...
      `Mode=${opts.mode} MASTER=${opts.masterEnabled}. Desk never auto-trades. Scout never live-buys. Live POST /api/buy needs chief:APPROVE. Only Grok Bot Bearer may POST /api/buy and /api/sell.`,
      ...
      "GET /api/opportunities first. Recite every open chance. Grade A and B: tell Taskra right away. The desk does not Approve. Wait for Taskra, Chief, or invited team. If the queue is empty, say so.",
      ...rungJobs(rung),
    ...
    never: [
      ...
      "Do not invent chief:APPROVE. Grok never sends it. The desk never Approves. Do not raise maxSolPerTrade or dailyBudgetSol unless Taskra says so. MASTER stays off unless Taskra resumes it.",
    ...
    howToWork: [
      ...
      "The desk never Approves and never trades. Taskra, Chief, invited team, or Grok Bot decide on Home. Majors wait for Taskra unless standing lessons already say what they would do.",
      "Keep searching even with no SOL / exhausted daily budget — queue the chance; do not stop the hunt. Aim to catch gems early.",
      "For Solana names: Scout + Intel. High hype+volume → queue GET /api/opportunities. Do not POST /api/buy live until Taskra, Chief, or team has APPROVE on Home or {chief:\"APPROVE\"}. The desk never fills on its own.",
