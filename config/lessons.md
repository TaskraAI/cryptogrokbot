...
- 2026-08-26T17:10:00.000Z GROK DECIDES + MOON BAG (user override of size-ask): do **not** wait for Taskra on good hype+volume names. Grok Bot buys at `maxSolPerTrade` (0.05 SOL). Cost-out initial SOL at **2–5x** (Grok Bot picks from the coin). Then hold the moon bag for 50–100x attempts. Learn from misses (`missed_gem`). MASTER stays off; auto live still blocked — Grok Bot Bearer `POST /api/buy`. Do not raise size. Polymarket still off. Not a promise.
