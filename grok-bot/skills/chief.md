...
If `https://cryptogrokbot.com/health` is 530/1016, this computer is still the origin. Use `http://127.0.0.1:8787` with the invite Bearer (do not wait on the public hostname). Run `scripts/start-desk.sh`. That starts `npm run agent`, the named tunnel `cryptogrokbot-dashboard`, and a localhost.run origin the Worker can fetch (trycloudflare `--url` hostnames 530 from this VM; the API token cannot write a tunnel CNAME). Taskra must place `CLOUDFLARE_API_TOKEN` in `/tmp/cf-api.token` (mode 0600, never chat). Live signs need `WALLET_SECRET_KEY` in gitignored `.env`. Never invent those secrets.
